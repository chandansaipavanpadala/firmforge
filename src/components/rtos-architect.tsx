...
import type { RTOSTemplate } from "@/lib/prompt-templates";
...
function StatusDot({ status }: { status: FileStatus }) {
  if (status === "idle") {
    return <span className="w-2 h-2 rounded-full bg-[var(--muted-foreground)]/50 flex-shrink-0" />;
  }
  if (status === "streaming") {
    return (
      <svg
...
export function RTOSArchitect({ initialPrompt }: RTOSArchitectProps) {
  ...
  const handleTemplateSelect = useCallback((template: RTOSTemplate) => {
    setDescription(template.prompt);
    setMcu(template.mcu);
    setRtos(template.rtos);
  }, []);

  // ── History restore handler ───────────────────────────────────────────
  const handleHistoryRestore = useCallback((entry: HistoryEntry) => {
    try {
      const parsed = JSON.parse(entry.code);
      setFiles(parsed);
      setRawStream(""); // Clear raw stream since we're restoring
    } catch {
      // If it's not JSON (single code string), put it in main.c
      setFiles({ "main.c": entry.code, "tasks.h": "", "config.h": "" });
    }
    setError(null);
  }, []);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const finalFiles = parseFiles(accumulated);
      const totalLines = Object.values(finalFiles).reduce(
        (sum, content) => sum + (content ? content.split("\n").length : 0),
        0
      );
      addEntry({
        type: "rtos",
        ...
        mcu,
        secondary: rtos,
        code: JSON.stringify(finalFiles),
  ...
  }, [description, mcu, rtos, options, addEntry]);
  ...
  const handleShare = useCallback(async () => {
    if (!hasAnyContent || !shareEnabled) return;
    setIsSharing(true);
    try {
      const shortId = await createShareLink(JSON.stringify(files), {
        type: "rtos",
        mcu,
        rtos,
  ...
  }, [hasAnyContent, shareEnabled, files, mcu, rtos]);
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="e.g. Read temperature from DHT22 every 2 seconds using a FreeRTOS task, apply a moving average filter, and publish via MQTT over WiFi. Alert via GPIO if temp exceeds 35C."
              rows={5}
              className="bg-[var(--card)] border-[var(--border)] text-[var(--foreground)] placeholder:text-[var(--muted-foreground)]/60 focus-glow resize-none"
              style={{
          ...
          {isLoading && (
            <div className="w-full h-0.5 bg-[var(--muted)] rounded-full overflow-hidden">
              <motion.div
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 text-xs font-medium rounded-lg transition-all cursor-pointer ${
                          isActive
                            ? "bg-[var(--primary)]/10 border border-[var(--primary)]/40 text-[var(--primary)]"
                            : "text-[var(--muted-foreground)] hover:text-[var(--foreground)] border border-transparent hover:border-[var(--border)]"
                        }`}
                        style={{ fontFamily: "var(--font-jetbrains-mono)" }}
                      >
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border text-sm transition-all ${
                      !activeContent
                        ? "border-[var(--border)]/50 bg-[var(--card)]/50 text-[var(--muted-foreground)]/50 cursor-not-allowed"
                        : "border-[var(--border)] bg-[var(--card)] text-[var(--foreground)] hover:border-[var(--primary)]/30 hover:bg-[var(--primary)]/5 cursor-pointer"
                    }`}
                    style={{ fontFamily: "var(--font-dm-sans)" }}
                  >
                    {copied ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-[var(--border)] bg-[var(--card)] text-[var(--foreground)] hover:border-[#7C3AED]/30 hover:bg-[#7C3AED]/5 transition-all text-sm cursor-pointer"
                    style={{ fontFamily: "var(--font-dm-sans)" }}
                  >
                    <svg
                      width="14"
